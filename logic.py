...
class PlayerServerInterface:
    ...
    def set_tangan(self, params=[]):
        pnum = params[0]
        tangan = params[1]
        try:
            self.players[pnum] = f"{tangan}"
            self.players.sync()
            return dict(status='OK', player=pnum)
        except Exception as e:
            return dict(status='ERROR')

    def get_tangan(self, params=[]):
        pnum = params[0]
        try:
            return dict(status='OK', tanganrad=self.players[pnum])
        except Exception as ee:
            return dict(status='ERROR')

if __name__ == '__main__':
    p = PlayerServerInterface()
    p.set_location(['1', 10, 10])
    print(p.get_location('1'))
    p.set_location(['2', 300, 300])
